...
function BuilderTab({ onOpenLead }) {
  ...
  const discountPct = kWh < 0.5 ? 0 : Math.min(25, Math.round(kWh * 8));
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 14px 40px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14, overflowX: "auto", paddingBottom: 2 }}>
        {[
          { thresh: 0.5, label: "הנחה 4%",    icon: "💸", reached: kWh >= 0.5, sub: "מעל 0.5kWh" },
          { thresh: 1.0, label: "הנחה 8%",    icon: "🎯", reached: kWh >= 1.0, sub: "מעל 1kWh" },
          { thresh: 1.5, label: "הנחה 12%",   icon: "🔥", reached: kWh >= 1.5, sub: "מעל 1.5kWh" },
          { thresh: 2.0, label: "הנחה 16%",   icon: "⚡", reached: kWh >= 2.0, sub: "מעל 2kWh" },
          { thresh: 3.0, label: "הנחה 24%",   icon: "💥", reached: kWh >= 3.0, sub: "מעל 3kWh" },
        ].map((tier, i) => (
          <div key={i} style={{
      ...
      <div style={{ background: C.card, borderRadius: 14, padding: "18px 16px", marginBottom: 12, border: `1px solid ${discountPct > 0 ? C.green + "44" : C.cyan + "30"}`, boxShadow: `0 0 30px ${discountPct > 0 ? C.green + "12" : C.cyan + "08"}` }}>
        ...
        {kWh < 3.1 && (
          <div style={{ marginTop: 10, background: C.elevated, borderRadius: 8, padding: "8px 12px", border: `1px solid ${C.cardBord}`, fontSize: 11, color: C.gray, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span>
              {(() => {
                const steps = [[0.5,4],[1,8],[1.5,12],[2,16],[2.5,20],[3,24]];
...
- מחיר: ~₪3,200 לkWh (+ 10% ל-BMS מעל 100A, + ₪180 לBluetooth)
- אחריות: 12 חודש מלאה
